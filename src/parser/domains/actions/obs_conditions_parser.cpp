...
ast::obs_cond obs_conditions_parser::parse_obs_cond(parser_helper &helper) {
    ...
    if (tok->has_type<observability_token::always>()  or
        ...
        obs_cond = obs_conditions_parser::parse_single_obs_cond(helper, false);
    else if (tok->has_type<connective_token::conjunction>())
        obs_cond = obs_conditions_parser::parse_obs_cond_list(helper);
...
ast::single_obs_cond obs_conditions_parser::parse_single_obs_cond(parser_helper &helper, bool parse_outer_pars) {
    if (parse_outer_pars) helper.check_next_token<punctuation_token::lpar>();
    const token_ptr &tok = helper.peek_next_token();
    ast::single_obs_cond obs_cond;

    if (tok->has_type<observability_token::always>())            obs_cond = obs_conditions_parser::parse_always_obs_cond(helper, false);
    else if (tok->has_type<observability_token::if_cond>())      obs_cond = obs_conditions_parser::parse_if_then_else_obs_cond(helper, false);
    else if (tok->has_type<quantifier_token::forall>())          obs_cond = obs_conditions_parser::parse_forall_obs_cond(helper, false);
...
//ast::simple_obs_cond obs_conditions_parser::parse_simple_obs_cond(parser_helper &helper) {
//    helper.check_next_token<punctuation_token::lpar>();
//    const token_ptr &tok = helper.peek_next_token();
...
ast::obs_cond_list obs_conditions_parser::parse_obs_cond_list(parser_helper &helper) {
    helper.check_next_token<connective_token::conjunction>();
    auto obs_conds = helper.parse_list<ast::single_obs_cond>([&] () { return obs_conditions_parser::parse_single_obs_cond(helper); });

    return obs_conds;
}

ast::always_obs_cond_ptr obs_conditions_parser::parse_always_obs_cond(parser_helper &helper, bool parse_outer_pars) {
    if (parse_outer_pars) helper.check_next_token<punctuation_token::lpar>();
    helper.check_next_token<observability_token::always>();
...
ast::if_then_else_obs_cond_ptr obs_conditions_parser::parse_if_then_else_obs_cond(parser_helper &helper, bool parse_outer_pars) {
    ...
    auto else_if_conds = helper.parse_list<ast::else_if_obs_cond_ptr, observability_token::else_cond>([&]() { return obs_conditions_parser::parse_else_if_obs_cond(helper); }, true);
