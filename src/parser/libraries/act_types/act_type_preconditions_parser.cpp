...
ast::event_precondition_list act_type_preconditions_parser::parse(epddl::parser_helper &helper) {
    ...
    return helper.parse_list<ast::event_precondition>([&] () { return act_type_preconditions_parser::parse_event_precondition(helper); }, true);    // todo: which list delimiter do we use?
}
