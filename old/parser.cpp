...
void parser::get_next_token() {
    ...
    if (std::get<punctuation_value>(m_current_token->get_type()) == punctuation_value::lpar) {
        ++m_lpar_count;
    } else if (std::get<punctuation_value>(m_current_token->get_type()) == punctuation_value::rpar) {
        if (m_lpar_count == m_scopes.top().first) {
            m_scopes.pop();
        }
        --m_lpar_count;
    } else if (m_lex.get_dictionary().is_scope(m_current_token->get_type())) {
        m_scopes.emplace(m_lpar_count, m_current_token->get_type());
    }
}
...
void parser::check_current_token(const token_type& expected_type) {
    if (!m_current_token->has_type(expected_type)) {
        std::string type;
        ...
        throw EPDDLException{std::string{""}, m_current_token->get_row(), m_current_token->get_col(), std::string{"Expected " + type + std::string{"."}}};
    }
}
...
template<class token_type>
std::unique_ptr<token_type> parser::get_last_node_from_token_list(const std::list<token_type>& to_check) {
...
template<class token_type>
std::unique_ptr<token_type> parser::parse_basic_or_rpar(basic_value type) {
    peek_next_token();
...
domain_item parser::parse_domain_item() {
    ...
    if (m_current_token->has_type(keyword_value::act_type_lib)) {
        item = parse_domain_act_type_libs();
    } else if (m_current_token->has_type(keyword_value::requirements)) {
        item = parse_domain_requirements();
    } else if (m_current_token->has_type(keyword_value::types)) {
        item = parse_domain_types();
    } else if (m_current_token->has_type(keyword_value::predicates)) {
        item = parse_domain_predicates();
    } else if (m_current_token->has_type(keyword_value::modalities)) {        // todo: implement new idea on modalities
        item = parse_domain_modalities();
    } else if (m_current_token->has_type(keyword_value::action)) {
        item = parse_action();
    } else {
        throw EPDDLException{std::string{""}, m_current_token->get_row(), m_current_token->get_col(), std::string{"Expected keyword."}};
    }
